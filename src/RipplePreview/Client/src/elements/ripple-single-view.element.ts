...
import { UMB_BLOCK_SINGLE_ENTRY_CONTEXT, UMB_BLOCK_SINGLE_MANAGER_CONTEXT } from '@umbraco-cms/backoffice/block-single';
...
/**
 * Single Block editor custom view (Umbraco 17.3+): live server-rendered preview.
 */
@customElement('ripple-single-view')
export class RippleSingleViewElement extends RippleBaseViewElement {
	protected readonly editorKind = 'single';

	#entry?: typeof UMB_BLOCK_SINGLE_ENTRY_CONTEXT.TYPE;
	...
	constructor() {
		...
		this.consumeContext(UMB_BLOCK_SINGLE_ENTRY_CONTEXT, (entry) => {
			this.#entry = entry;
		});

		this.consumeContext(UMB_BLOCK_SINGLE_MANAGER_CONTEXT, (manager) => {
			if (!manager) return;
	...
	protected buildRequest(): RippleRenderRequest | null {
		...
		return {
			blockValue: {
				layout: { 'Umbraco.SingleBlock': [this.layout] },
...
export default RippleSingleViewElement;

declare global {
	interface HTMLElementTagNameMap {
		'ripple-single-view': RippleSingleViewElement;
	}
}
